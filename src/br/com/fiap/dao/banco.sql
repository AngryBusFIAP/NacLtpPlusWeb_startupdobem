...
ALTER TABLE T_SCN_RECLAMACAO
ADD CONSTRAINT TX_SENTIDOVIAGEM CHECK (UPPER(tx_sentidoviagem) IN ('B', 'T', 'M', 'C'));

ALTER TABLE T_SCN_RECLAMACAO
ADD CONSTRAINT NR_NOTAPOSRECLAM DEFAULT 0;

ALTER TABLE T_SCN_REGIAO
ADD CONSTRAINT nm_regiao CHECK (UPPER(nm_regiao) in 
('N', 'NO', 'NE', 'S', 'SE', 'SO', 'O', 'C', 'CO', 'L' ));

ALTER TABLE T_SCN_PESSOA
ADD CONSTRAINT dt_nasc CHECK (dt_nasc < '01/01/1999');

ALTER TABLE T_SCN_CLIENTE
ADD CONSTRAINT tx_sexo CHECK(UPPER(tx_sexo) IN ('M', 'F'));